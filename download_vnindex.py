...
# -*- coding: utf-8 -*-
"""
Script để download dữ liệu lịch sử của chỉ số VNINDEX
...
def download_vnindex_data(
    ...
    """
    Download dữ liệu lịch sử cho VNINDEX với retry logic
...
def download_vnindex(
    ...
    print(f"[INFO] Bắt đầu download VNINDEX từ {start_date} đến {end_date}")
    print(f"[INFO] Thư mục output: {output_path.absolute()}")
    print(f"[INFO] Sleep: {sleep}s")
    
    # Kiểm tra file đã tồn tại
    output_file = output_path / "VNINDEX.parquet"
    ...
    # Download
    print("[INFO] Đang download VNINDEX...")
    result = download_vnindex_data(start_date, end_date, output_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="Download dữ liệu lịch sử chỉ số VNINDEX"
    )
    parser.add_argument(
        "--start",
        type=str,
